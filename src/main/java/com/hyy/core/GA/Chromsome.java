...
/**
 * Created by HengYanYan on 2016/5/2  16:31
 */
@Component
public class Chromsome {
    ...
    /********************************    Step  1.1.1   初始化染色体     ******************************/
    public Chromsome(Integer chromSize,double[][] distance) { //初始化静态成员变量，所有对象共用
    ...
    public String inialChrom(int chromSize) {
        ...
        for (int i = 0; i < chromSize; i++) {
            allowedChroms.add(chars.charAt(i));//备选基因组
        }
    ...
    public static Double getFitness(String path) {
        int i, j, temp = chars.indexOf(path.charAt(0));
        Double len = distance[0][temp+1];  //路径长加上起点到染色体0编码位置对应的地点的路程
        for (int index = 1; index < path.length(); index++) {
            i = temp;
            j = chars.indexOf(path.charAt(index));
            temp = j;
    ...
    public int getLeftCity(int position){
        if(position==0){
            return -1;
        }
        return this.getCity(position - 1);
    }
    public int getRightCity(int position){
        if(position == chromSize-1){
            return -2;
        }
        return this.getCity(position + 1);
    }
    public int getCity(int position){
        return Chromsome.chars.indexOf(this.getPath().charAt(position));
    }
